...
export const NewsFinderLeftSidebar = () => {
  const navigate = useNavigate();
  const trendingTopics = [
    { label: '🚀 AI Innovation', query: 'Artificial Intelligence' },
    { label: '🌍 Climate Tech', query: 'Climate Technology' },
    { label: '📈 Global Markets', query: 'Global Markets' },
    { label: '🔋 Clean Energy', query: 'Clean Energy' },
    { label: '🛰️ Space Exploration', query: 'Space Exploration' },
  ];
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-4 border border-blue-900/30">
        ...
        <div className="space-y-2">
          {trendingTopics.map((item, idx) => (
            <button
              key={idx}
              onClick={() => navigate(`/newsfinder?q=${encodeURIComponent(item.query)}`)}
...
export const NewsFinderRightSidebar = () => {
  const navigate = useNavigate();
  const [history, setHistory] = useState([]);

  const loadHistory = () => {
    try {
      const saved = localStorage.getItem('newscloud_search_history');
  ...
  useEffect(() => {
    loadHistory();
    window.addEventListener('search_history_updated', loadHistory);
    return () => window.removeEventListener('search_history_updated', loadHistory);
  }, []);

  const handleRemove = (e, queryToRemove) => {
    e.stopPropagation();
    if (window.confirm(`Are you sure you want to remove "${queryToRemove}" from your recent searches?`)) {
      ...
      {history.length > 0 ? (
        <div className="space-y-2">
          {history.map((item, idx) => {
            ...
            return (
              <div key={idx} className="w-full flex items-center bg-gray-700/30 hover:bg-gray-700/80 border border-gray-600/30 hover:border-blue-500/50 rounded-lg transition-all group">
                <button
                  onClick={() => navigate(`/newsfinder?q=${encodeURIComponent(query)}`)}
                  className="flex-1 flex items-center justify-between text-left px-3 py-2 text-sm text-gray-300 group-hover:text-blue-300 truncate"
                >
        ...
        <p className="text-sm text-gray-500 italic text-center py-4">No recent searches.</p>
      )}
    </div>
  );
};
